<template>
  <section class="bg-white">
    <div class="recruitment-form-container">
      <h2 class="text-xl font-bold mb-4 underline">প্রশিক্ষণ</h2>
      <!-- <p class="mb-6">
      ...
      <form @submit.prevent class="sm:w-[900px]">
        <div class="mx-auto">
          ...
          <div class="flex justify-end -mb-6 md:mb-6">
            <button @click="addTraining" class="base-blank-btn mr-4">
              Add +
            </button>
          </div>
          ...
          >
            ...
            >
              ...
              <div
                ...
                :key="input.id"
                :class="[
                  input.name === 'training_description' ? 'sm:col-span-3' : '',
                ]"
              >
                ...
                <input
                  v-if="
                    ...
                    input.name !== 'training_certificate'
                  "
                  :id="input.id"
                  :placeholder="input.placeholder"
                  class="p-[14px] border rounded-md focus:outline-hidden focus:border-blue-500 w-full"
                  v-model="inputValues[rowIndex][input.name]"
                />

                <FileUpload
                  v-if="input.name === 'training_certificate'"
                  :inputId="`trainingCertificate${input.id}`"
                  label=""
                  v-model="inputValues[rowIndex][input.name]"
                  :uploadedFile="inputValues[rowIndex][input.name]"
                />

                <div v-if="input.name === 'training_description'">
                ...
                <div class="dynamic-action-container">
                  <button
                    ...
                    :disabled="false"
                    @click="deleteTraining(false, rowIndex)"
                    :class="[
                  ...
                  <button
                    v-if="inputValues[rowIndex]['id'] && inputIndex === 5"
                    @click="
                      updateTraining({
      ...
      <Navigation
        :prevStep="prevStep"
        :submit-function="() => nextStep(false)"
        :isLoading="isLoading"
      />
    </div>
  </section>
...
<script setup>
  ...
  let inputs = ref(useStore.inputsStage5);
  let inputValues = ref(useStore.inputValuesStage5);
  ...
  async function syncTrainingInfo() {
    await useUserInfo.getProfileInfo();
    if (useUserInfo.userInfo?.user_trainings.length == 0) return;
    let newRow =
      useUserInfo.userInfo?.user_trainings.length - inputValues.value.length;
    ...
    if (
      ...
      useUserInfo.userInfo?.user_trainings.map((item, index) => {
  ...
  const deleteTraining = async (isModal, index) => {
    if (!isModal) {
      ...
      if (id !== undefined) {
        try {
          let payload = { id };
          const endpoint = "/user/delete-trainings";
          const { data, error } = await callAuthnAxios(endpoint, payload);
  ...
  async function updateTraining({ id, index }) {
    let training = inputValues.value[index];

    try {
      const endpoint = "/user/update-trainings";

      let payload = {
        id: training.id,
        training_name: training.training_name,
        trainer_organization: training.trainer_organization,
