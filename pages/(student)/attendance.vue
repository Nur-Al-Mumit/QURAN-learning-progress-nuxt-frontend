  ...
  <div class="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-4 sm:p-6">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-4 sm:p-6 border border-gray-100">
          ...
          <p class="mt-2 text-2xl sm:text-3xl font-bold text-indigo-600">{{ monthlyAttendance }}%</p>
          <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ attendedFridays }} out of {{ totalFridays }} Fridays</p>
        </div>
        <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-4 sm:p-6 border border-gray-100">
          <h3 class="text-base sm:text-lg font-medium text-gray-900 flex items-center">
          ...
          <p class="mt-2 text-2xl sm:text-3xl font-bold text-green-600">{{ totalClassesAttended }}</p>
          <p class="text-xs sm:text-sm text-gray-500 mt-1">Out of {{ totalClasses }} classes</p>
        </div>
        <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-4 sm:p-6 border border-gray-100">
          <h3 class="text-base sm:text-lg font-medium text-gray-900 flex items-center">
          ...
          <p class="mt-2 text-2xl sm:text-3xl font-bold text-blue-600">{{ currentStreak }} weeks</p>
          <p class="text-xs sm:text-sm text-gray-500 mt-1">Consecutive attendance</p>
        </div>
      </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div
              ...
              :class="{
                ...
                'bg-red-50 border-red-200': !friday.attended && friday.date < today,
                'bg-gray-50 border-gray-200': !friday.attended && friday.date >= today,
              }"
            >
              <span class="text-sm font-medium text-gray-700">Week {{ friday.weekNumber }}</span>
              <p class="text-xs text-gray-500 mt-1">{{ friday.formattedDate }}</p>
              <p class="text-xs mt-2" :class="friday.attended ? 'text-green-600' : 'text-gray-500'">
                {{ friday.attended ? 'Present' : friday.date < today ? 'Absent' : 'Upcoming' }}
              </p>
              <button
                ...
                :class="friday.attended ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-green-100 text-green-600 hover:bg-green-200'"
              >
          ...
          <div class="overflow-x-auto rounded-lg border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="(record, index) in attendanceRecords.slice(0, 5)" :key="index" class="hover:bg-gray-50 transition-colors duration-150">
...
<script setup>
  ...
  const fridaysInMonth = computed(() => {
    ...
    while (firstFriday.getMonth() === month) {
      const attendanceRecord = attendanceRecords.value.find(
        record => new Date(record.date).toDateString() === firstFriday.toDateString()
      );
  ...
  const attendedFridays = computed(() => fridaysInMonth.value.filter(f => f.attended).length);
